...
public class TodoController {
    ...
    public String showTodos(ModelMap map) {
        ...
        List<TodoEntity> todosList = todoService.findByUserName(userName);
    ...
    public String addTodo(ModelMap map) {
        map.addAttribute("todo", new TodoEntity(0, getLoggedInUserName(), "", LocalDate.now(), false));
        return "addTodo";
    }

    @RequestMapping(value = "add-todo", method = RequestMethod.POST)
    public String showNewTodo(ModelMap map, @Valid TodoEntity todo, BindingResult result) {
        if(result.hasErrors()) {
            return "addTodo";
        }
    ...
    public String updateTodo(ModelMap map, @RequestParam Integer id) {
        map.addAttribute("todo", new TodoEntity(id, getLoggedInUserName(), "", LocalDate.now(), false));
        return "updateTodo";
    }

    @RequestMapping(value = "update-todo", method = RequestMethod.POST)
    public String updateTodoRedirect(ModelMap map, @RequestParam Integer id, TodoEntity todo) {
